...
def stock_list(request, *args, **kwargs):
	...
	openings = Stock.objects.filter(Q(business=business, stock_date__month=dt.month, stock_type='Opening Stock')|Q(business=business, stock_date__month=dt.month+1, stock_type='Opening Stock')).order_by('-id')
	closings = Stock.objects.filter(business=business, stock_date__month=dt.month, stock_type='Closing Stock').order_by('-id')
	context = {
		'business':business,
		'products':products,
		'openings':openings,
...
def sales_list(request, *args, **kwargs):
	id = kwargs.get('id')
	business = Business.objects.filter(id=id).first()
	sales = Sale.objects.filter(Q(business=business, created_at__date=date.today())|Q(business=business, status='Awaiting Payment')).all().order_by('-pk')
	context = {
		'business':business,
		'sales':sales,
	}
...
def add_sale(request, *args, **kwargs):
	...
	if request.method == 'POST':
		form = SaleForm(request.POST or None, business=business)
						...
						if remain >= 0:
							...
							if sale.status == 'Completed':
								...
								try:
									...
									CheckAccount.objects.create(business=business, employee=request.user, description="Sale", debit=0, credit=sale.amount_paid, balance=balance)
								except:
									balance = Decimal(sale.amount_paid)
									CheckAccount.objects.create(business=business, employee=request.user, description="Sale", debit=0, credit=sale.amount_paid, balance=balance)
							
							return redirect('business:sales_list', id=business.id)
						else:
		...
		form = SaleForm(business=business, initial={'order_no':code})
...
def edit_sale(request, *args, **kwargs):
	...
	if request.method == 'POST':
		form = SaleForm(request.POST or None, business=sale.business, instance=sale)
		...
		if form.is_valid():
				...
				if sale.status == 'Completed':
					# Adding Transaction in Saving Account
					acc = CheckAccount.objects.filter(business=sale.business).last()
					try:
						balance = acc.balance + Decimal(sale.amount_paid)
						CheckAccount.objects.create(business=sale.business, employee=request.user, description="Sale", debit=0, credit=sale.amount_paid, balance=balance)
					except:
						balance = Decimal(sale.amount_paid)
						CheckAccount.objects.create(business=sale.business, employee=request.user, description="Sale", debit=0, credit=sale.amount_paid, balance=balance)

				return redirect('business:sales_list', id=sale.business.id)
		else:
			pass

	else:
		form = SaleForm(business=sale.business, instance=sale)
...
def deduct_sdl(**kwargs):

	business = kwargs.get('business')
	worker = kwargs.get('worker')
	gross_salary = kwargs.get('gross_salary')

	###### Check if employee object exists in Takehome
	employee = Takehome.objects.filter(business=business, payroll=worker).first()

	if employee:
		employee_salary = employee.salary 
		sdl = gross_salary * 0.04
		worker.sdl_amount = int(sdl)
		net_salary = employee_salary - sdl
		employee.salary = net_salary
		employee.save()
		worker.save()

	else:
		employee = Takehome.objects.create(payroll=worker, salary=gross_salary, business=business)
		sdl = gross_salary * 0.04
		worker.sdl_amount = int(sdl)
		net_salary = employee.salary - sdl
		employee.salary = net_salary
		employee.save()
		worker.save()


def deduct_loan(**kwargs):
...
def deduct_deduction(**kwargs):

	deduction = kwargs.get('deduction')
	employee = kwargs.get('employee')
	takehome_salary = employee.salary
	employee.salary = takehome_salary - deduction
	employee.save()



def deduct_nssf(**kwargs):
	...
	if employee:
		employee_salary = employee.salary 
		nssf = gross_salary * 0.1
		worker.nssf_amount = int(nssf)
		net_salary = employee_salary - nssf
		employee.salary = net_salary
		employee.save()
		worker.save()
		...
		nssf = gross_salary * 0.1
		worker.nssf_amount = int(nssf)
		net_salary = employee.salary - nssf
		employee.salary = net_salary
		employee.save()
		worker.save()


def deduct_heslb(**kwargs):
	...
	if employee:
		employee_salary = employee.salary 
		heslb = gross_salary * 0.15
		worker.heslb_amount = int(heslb)
		net_salary = employee_salary - heslb
		employee.salary = net_salary
		employee.save()
		worker.save()
		...
		heslb = gross_salary * 0.15
		worker.heslb_amount = int(heslb)
		net_salary = employee.salary - heslb
		employee.salary = net_salary
		employee.save()
		worker.save()
...
def takehome(request, *args, **kwargs):
	...
	if request.method == 'POST':
		...
		if payroll_form.is_valid():
			...
			if bonus or overtime:
				gross_salary = income + bonus + overtime 
			...
			if paye_rate:
				...
				if gross_salary <= 270000:                    
					...
					if employee:
						employee = Takehome.objects.filter(business=business, payroll=worker).first()
						employee.update(salary=gross_salary)

					else:		
						Takehome.objects.create(payroll=worker, salary=gross_salary, business=business)

					check_account_debit(business=business, amount=gross_salary, user=request.user, description="Payroll")


				elif 270000 < gross_salary <= 520000:
					
					tax_payment = (gross_salary - 270000) * 0.09 
					net_salary = gross_salary - tax_payment
					...
					if employee:
						employee = Takehome.objects.filter(business=business, payroll=worker).first()
						employee.update(salary=net_salary)

					else:		
						Takehome.objects.create(payroll=worker, salary=net_salary, business=business)

					check_account_debit(business=business, amount=income, user=request.user, description="Payroll")



				elif 520000 < gross_salary <= 760000:
					
					tax_payment = (gross_salary - 520000) * 0.2 + 22500
					net_salary = gross_salary - tax_payment 
				...
				elif 760000 < gross_salary <= 1000000:
					
					tax_payment = (gross_salary - 760000) * 0.25 + 70500
					net_salary = gross_salary - tax_payment
				...
				elif 1000000 < gross_salary:
					
					tax_payment = (gross_salary - 1000000) * 0.3 + 130500 
					net_salary = gross_salary - tax_payment
			...
			if sdl:
				deduct_sdl(worker=worker, gross_salary=gross_salary, business=business)

			if heslb:
				deduct_heslb(worker=worker, gross_salary=gross_salary, business=business)

			if nssf:
				deduct_nssf(worker=worker, gross_salary=gross_salary, business=business)
			...
			if deduction:
				if employee:
					deduct_deduction(employee=employee, deduction=deduction)

			#### Deduct loan return from employee
			if loan:
				if employee:				
					if loan.debt == True :
						deduct_loan(employee=employee, worker=worker, loan=loan)					
			...
			nssf_funds = Payroll.objects.filter(business=business).aggregate(Sum('nssf_amount'))['nssf_amount__sum']
			# wcf = Payroll.objects.filter(tax_rate__icontains='wcf', business=business)
			loan_board_funds = Payroll.objects.filter(business=business).aggregate(Sum('heslb_amount'))['heslb_amount__sum']
...
def expense_update(request, *args, **kwargs):
	...
	if request.method == 'POST':
		expense_form = ExpenseForm(request.POST or None, instance=expense_obj, request=request)
		...
		expense_form = ExpenseForm(instance=expense_obj, request=request) 
...
def expense_list(request, *args, **kwargs):
	...
	banks = Bank.objects.all().first()

	if request.method == 'POST':
		expense_form = ExpenseForm(request.POST, request=request)
		...
		expense_form = ExpenseForm(request=request)

		if request.is_ajax():
			expense_form = ExpenseForm(request=request)
			# print(expense_form.fields['account'].queryset)
			accounts = []
			for account in expense_form.fields['account'].queryset:
...
def add_supplier(request, *args, **kwargs):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			supplier.save()
			return redirect('business:suppliers')
	else:
		form = SupplierForm()
	...
	if request.method == "POST":
		...
		if purchase_order_form.is_valid() and purchase_order_list_form.is_valid():
			...
			if purchase_order.published:
				...
				from_email = 'info@clasktanzania.co.tz'
				recipient_list=[]
				for user in users:
		...
		if transfer == 'True':
			...
			if account_form.is_valid():
				...
				if check_account_balance:
					account_form.description = 'Depositing  Balance in Check Account'
					account_form.business = business
					account_form.employee = user
					account_form.credit = amount
					account_form.debit = 0.00					
					account_form.balance = int(check_account_balance) + int(amount)					
					account_form.save()
					return redirect('business:check-account', id=business.id, trans=False)
				else:	
	...
	if request.method == 'POST':
		...
		if transfer == 'True':
			...
			if account_form.is_valid():
				...
				account_form = account_form.save(commit=False)
				if saving_account_balance:
					account_form.description = 'Depositing Balance in Saving Account'
					account_form.business = business
					account_form.employee = user
					account_form.credit = amount
					account_form.debit = 0.00					
					account_form.balance = int(saving_account.balance) + int(amount)
					account_form.save()
					return redirect('business:saving-account', id=business.id, trans=False)
				else:					
					account_form.description = 'Opening Balance'
...
def add_stock(request,*args, **kwargs):
	...
	business = Business.objects.filter(id=id).first()
	if request.method == 'POST':
		form = StockForm(request.POST or None, business=business)
		if form.is_valid():
			stock = form.save(commit=False)
			stock.business = business
			stocks = Stock.objects.filter(business=business, product=stock.product, stock_date__month=stock.stock_date.month, stock_type=stock.stock_type).count()

			if stocks == 0:
				if stock.stock_type == 'Closing Stock' and stock.stock_date.month == dt.month:					
					stock.save()
					return redirect('business:stock_list', id=business.id)
				elif stock.stock_type == 'Opening Stock' and dt.month <= stock.stock_date.month <= dt.month+1:
					stock.save()
					return redirect('business:stock_list', id=business.id)	
				else:	
					messages.error(request, "Please select accurate date.")	
			else:	
				messages.error(request, "Stock already exist.")
				
	else:	
		form = StockForm(business=business)
	context = {
		'business':business,
		'form':form,
	}
...
def get_stock(request,*args, **kwargs):
	if request.is_ajax():
		...
		if id !='':
			product = Product.objects.filter(id=id).first()			
			inventory_qs = Inventory.objects.filter(product=product, exist=True).annotate(available=F('remain')-F('damage'))
			if inventory_qs:
				inventory_qs.filter(available=0).update(exist=False)		
			inventories = inventory_qs.filter(available__gt=0).order_by('-pk')
			total_cost = inventory_qs.aggregate(total_cost=Sum(F('available')*F('product_cost'),output_field=FloatField()))['total_cost']	

			data_dict = {
				'quantity':product.quantity if product != None else 0,
				'total_cost':float(total_cost)if total_cost != None else 0,
			}
...
@finance_required
@login_required
def products_sells(request, *args, **kwargs):
	id = kwargs.get('id')
	business = Business.objects.filter(id=id).first()
	products = Product.objects.filter(business=business).all()
	context = {
	...
	return render(request, "products_sells.html", context)


@finance_required
@login_required
def add_sell_price(request, *args, **kwargs):
	id = kwargs.get('id')
	product = Product.objects.filter(id=id).first()
	if request.method == 'POST':
		form = SellPriceForm(request.POST or None, instance=product)

		if form.is_valid():
			product.save()
			return redirect('business:products_sells', id=product.business.id)
	else:    
		form = SellPriceForm(instance=product)
	...
	return render(request, "add_sell_price.html", context)   


@store_required
@login_required
def goods_received_note(request, *args, **kwargs):
	id = kwargs.get('id')
	business = Business.objects.filter(id=id).first()
	grns = GoodsReceivedNote.objects.filter(Q(business=business, created_at__date=date.today())|Q(business=business, published=False)).all()
	context = {
		'business':business,
		'grns':grns,
	}
	return render(request, 'goods_received_note.html', context)


@store_required
@login_required
def add_goods_received_note(request, *args, **kwargs):
	id = kwargs.get('id')
	business = Business.objects.filter(id=id).first()
	data = datetime.now()
	if request.method == "POST":
		grn_form = GoodsReceivedNoteForm(request.POST or None, business=business)
		grn_list_form = GoodsReceivedNoteListFormSet(request.POST or None,)
		if grn_form.is_valid() and grn_list_form.is_valid():
			grn = grn_form.save(commit=False)
			grn.business = business
			grn.employee = request.user
			grn.save()

			grn_list_form.instance = grn
			grn_list_form.save()
			return redirect('business:goods_received_note', id=business.id)
	else:
		grn_form = GoodsReceivedNoteForm(business=business)
		grn_list_form = GoodsReceivedNoteListFormSet()
	context = {
		'business':business,
		'grn_form':grn_form,
		'purchase_order_list_form':grn_list_form,
	}
	return render(request, 'add_goods_received_note.html', context)


def autocomplete_order_no(request, *args, **kwargs):
	if request.is_ajax():
		id = kwargs.get('id')
		num = request.GET.get('term')

		purchase_order = PurchaseOrder.objects.filter(po_no__icontains=num, business=id, published=True, purchase_order_authorize__authorized=True,).all()
		local_purchase_order = LocalPurchaseOrder.objects.filter(lpo_no__icontains=num, business=id, published=True, local_purchase_order_authorize__authorized=True,).all()

			 
		results = []
		if purchase_order:
			for p in purchase_order:
				results.append(p.po_no)

		if local_purchase_order:
			for l in local_purchase_order:
				results.append(l.lpo_no)		
...
@procurement_finance_ceo_required
@login_required
def view_goods_received_note(request, *args, **kwargs):
	id = kwargs.get('id')
	grn = GoodsReceivedNote.objects.filter(id=id).first()
	context = {
		'business':grn.business,
		'grn':grn,
	}
	return render(request, 'view_goods_received_note.html', context)


@store_required
@login_required
def edit_goods_received_note(request, *args, **kwargs):
	id = kwargs.get('id')
	grn = GoodsReceivedNote.objects.filter(id=id).first()
	if request.method == "POST":
		grn_form = GoodsReceivedNoteForm(request.POST or None, business=grn.business, instance=grn)
		grn_list_form = GoodsReceivedNoteListUpdateFormSet(request.POST or None, instance=grn)
		if grn_form.is_valid() and grn_list_form.is_valid():
			grn_form.save()
			grn_list_form.save()
			
			return redirect('business:goods_received_note', id=grn.business.id)
		else:
			pass
	else:
		grn_form = GoodsReceivedNoteForm(instance=grn, business=grn.business)
		grn_list_form = GoodsReceivedNoteListUpdateFormSet(instance=grn)
	context = {
		'business':grn.business,
		'grn_form':grn_form,
		'purchase_order_list_form':grn_list_form,
	}
	return render(request, 'edit_goods_received_note.html', context)


@store_required
@login_required
def purchase_ledger(request, *args, **kwargs):
	id = kwargs.get('id')
	business = Business.objects.filter(id=id).first()
	ledgers = PurchaseLedger.objects.filter(Q(business=business, created_at__date=date.today())|Q(business=business, status="Awaiting Payment")).all()
	context = {
		'business':business,
		'ledgers':ledgers,
	}
	return render(request, 'purchase_ledger.html', context)	


@store_required
@login_required
def add_purchase_ledger(request, *args, **kwargs):
	id = kwargs.get('id')
	business = Business.objects.filter(id=id).first()
	if request.method == "POST":
		ledger_form = PurchaseLedgerForm(request.POST or None, business=business)
		if ledger_form.is_valid():
			ledger = ledger_form.save(commit=False)
			ledger.business = business
			ledger.employee = request.user
			ledger.save()
			
			return redirect('business:purchase_ledger', id=business.id)
	else:
		ledger_form = PurchaseLedgerForm(business=business)

	context = {
		'business':business,
		'ledger_form':ledger_form,
	}
	return render(request, 'add_purchase_ledger.html', context)


@store_required
@login_required
def edit_purchase_ledger(request, *args, **kwargs):
	id = kwargs.get('id')
	ledger = PurchaseLedger.objects.filter(id=id).first()
	if request.method == "POST":
		ledger_form = PurchaseLedgerForm(request.POST or None, business=ledger.business, instance=ledger)
		if ledger_form.is_valid():
			ledger_form.save()
			
			return redirect('business:purchase_ledger', id=ledger.business.id)
		else:
			pass
	else:
		ledger_form = PurchaseLedgerForm(instance=ledger, business=ledger.business)
	context = {
		'business':ledger.business,
		'ledger_form':ledger_form,
	}
	return render(request, 'edit_purchase_ledger.html', context)
...
@finance_required
@login_required
def equity_list(request, *args, **kwargs):

	equities = Equity.objects.filter(created_at__date=date.today())  

	if request.method == 'POST':
		equity_form = EquityModelForm(request.POST)

		if equity_form.is_valid():
			equity_form.save()
			return redirect('business:equity-list')

	else:
		equity_form = EquityModelForm()

	context = {
		'equities': equities,
		'equity_form': equity_form,
	}
	template_name = 'equity-list.html'
...
@finance_required
@login_required
def equity_delete(request, *args, **kwargs):
	id = kwargs.get('id')
	equity = Equity.objects.filter(id=id)  
