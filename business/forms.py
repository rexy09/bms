...
class ProductForm(forms.ModelForm):
    ...
    class Meta:
        model = Product
        fields = "__all__"
        exclude = ["business","currency","sell_price"]
...
class ExpenseForm(forms.ModelForm):  
    ...
    def __init__(self, *args,**kwargs):
        ...
        if bank is not None:
            self.fields['account'].queryset = Account.objects.filter(bank=int(bank))
            # print(self.fields['account'].queryset)

        else:
            self.fields['account'].queryset = Account.objects.filter(bank=bank)    
            self.fields['account'].empty_label = "Choose Account"    
...
class PayrollForm(forms.ModelForm):
    ...
    class Meta:
        model = Payroll
        fields = '__all__'
        exclude = ['business', 'tax_amount', 'paye_amount', 'sdl_amount', 'loan_debt', 'nssf_amount', 'heslb_amount']
...
class SellPriceForm(forms.ModelForm):
...
class GoodsReceivedNoteListForm(forms.ModelForm):
    
    class Meta:
        model = GoodsReceivedNoteList
        fields = '__all__'
        exclude = ['purchase_order',]


GoodsReceivedNoteListFormSet = forms.inlineformset_factory(GoodsReceivedNote, GoodsReceivedNoteList,form=GoodsReceivedNoteListForm, extra=3)
GoodsReceivedNoteListUpdateFormSet = forms.inlineformset_factory(GoodsReceivedNote, GoodsReceivedNoteList,form=GoodsReceivedNoteListForm, extra=0)


class PurchaseLedgerForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        self.business = kwargs.pop("business")
        super(PurchaseLedgerForm, self).__init__(*args, **kwargs)
        self.fields['supplier'].queryset = Supplier.objects.filter(business=self.business,)
        self.fields['supplier'].empty_label = "Select Supplier"
    ...
    class Meta:
        ...
        fields = '__all__'
        exclude = ['business','employee',]



class EquityModelForm(forms.ModelForm):
